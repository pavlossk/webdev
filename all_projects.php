...
if (!empty($_POST["epelekse"]) && $_SESSION["type"] == "student") {
    ...
    $sql1 = "SELECT students_number FROM projects WHERE projectID='$id'";
    $result1 = $conn->query($sql1);
    while ($row1 = $result1->fetch_assoc()) {
        if ($row1["students_number"] != $students_number) {
            $valid = 1;
        }
    }
    if ($valid == 0) {
        if($students_number==1){
            $sql = "INSERT INTO applications(projectID, student1,student2,student3, status) VALUES ('$id','$user','empty','empty','applied')";
        }else if($students_number==2){
            $sql = "INSERT INTO applications(projectID, student1,student2,student3, status) VALUES ('$id','$user','$second','empty','applied')";
        }else if($students_number==3){
            echo $second." ".$third;
            $sql = "INSERT INTO applications(projectID, student1,student2,student3, status) VALUES ('$id','$user','$second','$third','applied')";
        }
...
<body>
    <div class="container">
        ...
        if (!empty($_POST["ready"])) {
            ...
            $user = $_SESSION["username"];
            $sql = "SELECT teacher,projectname,applications.status as status FROM applications,projects WHERE projects.projectID=applications.projectID AND (applications.student1='$user' OR applications.student2='$user' OR applications.student3='$user')";
            $result = $conn->query($sql);
            $choice = 2;
        } else {
        }
